  ...
  function createCallback(rdio, callbackName, elementId) {
    ...
    callback.playingTrackChanged = function(playingTrack, sourcePosition) {
      if (playingTrack) {
        ...
        currentTrack.updateWithRdioTrack(playingTrack);

        // Update the playing header
        $('#current_track').show();
        
        // Notify the sinatra server that the next track has started so that it
        // alert the Hipchat chatroom.
        
        $.ajax({
           url: '/now_playing/' + encodeURIComponent(title),
           dataType: 'json'
        });
