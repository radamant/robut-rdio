...
class Robut::Plugin::Rdio
  ...
  class FindAction
    ...
    def find_results_for(sender,query_string)
      ...
      SearchResult.new @rdio, sender, results, query.terms, query_type.to_s.capitalize
    end
    ...
    def search_rdio(query,filters,start=0,count=10)
      ...
      # We also want to exclude the tracks that cannot be streamed from the resultset
    
      results.map do |result|
      
        result = (result.is_a?(::Rdio::Artist) ? result.tracks(nil,0,3) : result)
        result.can_stream ? result : nil
